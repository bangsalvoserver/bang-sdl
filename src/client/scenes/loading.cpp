...
loading_scene::loading_scene(game_manager *parent)
    ...
    , m_loading_tex(sdl::surface(GET_RESOURCE(icon_loading_png))) {}
...
void loading_scene::resize(int width, int height) {
    ...
    m_loading_rect = m_loading_tex.get_rect();
...
void loading_scene::render(sdl::renderer &renderer) {
    ...
    SDL_RenderCopyEx(renderer.get(), m_loading_tex.get_texture(renderer), nullptr, &m_loading_rect, m_loading_rotation, nullptr, SDL_FLIP_NONE);
