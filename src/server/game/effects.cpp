...
namespace banggame {
    void effect_bang::on_play(player *origin, player *target) {
        target->get_game()->add_response<response_type::bang>(origin, target);
    }

    void effect_indians::on_play(player *origin, player *target) {
        target->get_game()->add_response<response_type::indians>(origin, target);
    }

    void effect_duel::on_play(player *origin, player *target) {
        target->get_game()->add_response<response_type::duel>(origin, target);
    }
    ...
    void effect_generalstore::on_play(player *origin) {
        ...
        for (int i=0; i<origin->get_game()->num_alive(); ++i) {
            ...
            origin->get_game()->add_response<response_type::generalstore>(origin, target);
    ...
    void effect_jail::on_equip(player *target) {
        target->add_predraw_check<effect_jail>(1);
    }

    void effect_jail::on_unequip(player *target) {
        target->remove_predraw_check<effect_jail>();
    }

    void effect_jail::on_predraw_check(player *target_player, card *target_card) {
        target_player->get_game()->draw_check_then(target_player, [=](card *check) {
            target_player->discard_card(target_card);
            if (check->suit != card_suit_type::hearts) {
    ...
    void effect_dynamite::on_equip(player *target) {
        target->add_predraw_check<effect_dynamite>(2);
    }

    void effect_dynamite::on_unequip(player *target) {
        target->remove_predraw_check<effect_dynamite>();
    }

    void effect_dynamite::on_predraw_check(player *target_player, card *target_card) {
        target_player->get_game()->draw_check_then(target_player, [=](card *check) {
            if (check->suit == card_suit_type::spades
                && enums::indexof(check->value) >= enums::indexof(card_value_type::value_2)
                && enums::indexof(check->value) <= enums::indexof(card_value_type::value_9)) {
                target_player->damage(3);
                target_player->discard_card(target_card);
            } else {
                target_player->get_next_player().equip_card(target_player->get_card_removed(target_card));
            }
        });
    }
